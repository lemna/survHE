...
#'
#' @param formula a formula specifying the model to be used, of the form
#' \code{Surv(time, event) ~ treatment [+ covariates]} for flexsurv, or
#' \code{inla.surv(time, event) ~ treatment [+ covariates]} for INLA
#' @param data A data frame containing the data to be used for the analysis. This must contain data for
#' the \code{event} variable. In case there is no censoring, then \code{event} is a column of 1s.
...
#' @param ... Additional options (for INLA or HMC):
#' \itemize{
...
#'  \item prior var = 1000 for *all* fixed effects
...
#' @importClassesFrom Matrix dsparseMatrix replValueSp
#'
#' @examples
